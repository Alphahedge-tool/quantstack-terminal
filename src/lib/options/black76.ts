/**
 * Black-76 — browser copy.
 *
 * ── Why this is a copy ──
 *
 * The authority is `backend/analytics/black76.ts` in the qt-backend repo, and
 * this file is a verbatim duplicate of it. That is deliberate, and it is the
 * lesser of two evils.
 *
 * The payoff chart re-prices every leg on every scenario change — a spot slider,
 * an IV shift, a day of decay — which is hundreds of evaluations per frame at
 * 1 Hz or faster. Doing that over the network would put a round trip inside a
 * slider drag. So the math has to run in the browser.
 *
 * It could not simply be imported: the backend compiles as its own package with
 * a different target (ES2022 / NodeNext against this app's bundler resolution)
 * and lives in a separate repo, so reaching across pulls the whole Node build
 * into the browser graph.
 *
 * ── READ THIS BEFORE EDITING ──
 *
 * There are now THREE copies of this file: the backend authority, the Next.js
 * terminal's copy, and this one. The backend repo's `scripts/verifyOptionsMath.ts`
 * asserts the first two agree to 1e-12 across a grid of forwards, strikes,
 * tenors and vols — it does NOT know about this file, because it cannot import
 * across repos.
 *
 * So this copy is the one with no automated guard on it. It was taken byte-for-
 * byte from the terminal copy at port time and must be re-taken the same way.
 * Do not "improve" it here: change the backend original, propagate to both
 * copies, and re-run that check.
 */
